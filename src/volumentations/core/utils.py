...
class DataProcessor:
    ...
    def check_and_convert(self, data, rows, cols, direction="to"):
        if self.params.format == "volumentations":
            self.check(data, rows, cols)
            return data

        if direction == "to":
            return self.convert_to_volumentations(data, rows, cols)

        return self.convert_from_volumentations(data, rows, cols)
    ...
    @abstractmethod
    def convert_to_volumentations(self, data, rows, cols):
        pass

    @abstractmethod
    def convert_from_volumentations(self, data, rows, cols):
        pass
